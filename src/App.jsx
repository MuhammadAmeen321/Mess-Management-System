...
import {
  ...
  ChefHat, Lock, Bell, UserPlus, Sparkles
} from "lucide-react";
import {
...
function Complaints({ complaints, setComplaints, members }) {
  ...
  async function analyzeWithAI() {
    ...
    setAiResult(null);
    try {
      const res = await fetch("/api/analyze-complaints", {
        method: "POST",
      ...
      setAiError("Couldn't reach the AI assistant right now. Please try again in a moment.");
    } finally {
      setAiLoading(false);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="mm-btn-primary rounded px-4 py-2 text-sm font-semibold flex items-center gap-2"
            style={{ opacity: aiLoading || complaints.length === 0 ? 0.6 : 1 }}
          >
            <Sparkles size={16} /> {aiLoading ? "Analyzing..." : "Analyze with AI"}
          </button>
      ...
      {aiError && (
        <div className="mm-ledger-card rounded-md p-4 text-sm" style={{ color: "var(--rust-600)" }}>{aiError}</div>
      )}

      {aiResult && (
        <div className="mm-ledger-card rounded-md p-5" style={{ borderTop: "3px solid var(--maroon-800)" }}>
          <div className="flex items-center gap-2 mb-3">
            <Sparkles size={17} style={{ color: "var(--maroon-800)" }} />
            <h3 className="mm-serif text-lg font-semibold" style={{ color: "var(--maroon-900)" }}>AI Complaint Insights</h3>
          </div>
          {aiResult.summary && (
            <p className="text-sm mb-4" style={{ color: "var(--ink-700)" }}>{aiResult.summary}</p>
          )}
          <div className="flex flex-col gap-3">
            {(aiResult.themes || []).map((t, i) => (
              <div key={i} className="flex items-start justify-between gap-4 p-3 rounded" style={{ background: "var(--paper-100)" }}>
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-semibold text-sm" style={{ color: "var(--ink-900)" }}>{t.theme}</span>
                    <Pill tone={t.priority === "High" ? "Open" : t.priority === "Low" ? "Resolved" : "Open"}>{t.priority} priority</Pill>
                    <span className="text-xs mm-mono" style={{ color: "var(--ink-400)" }}>{t.count} complaint{t.count === 1 ? "" : "s"}</span>
                  </div>
                  <p className="text-sm" style={{ color: "var(--ink-600)" }}>{t.suggestedAction}</p>
                </div>
              </div>
            ))}
          </div>
